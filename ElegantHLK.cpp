...
bool IsAdvancedFiltered(const char *filename, LONGLONG fileSize)
{
    if (g_MinSize > 0 && fileSize < g_MinSize)
        return true;
    if (g_MaxSize > 0 && fileSize > g_MaxSize)
        return true;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        if (HIWORD(wParam) == BN_CLICKED)
        {
            ...
            case ID_BTN_ABOUT:
                MessageBoxA(hwnd, "Fork自https://github.com/Henglie/ElegantHLK", "关于软件", MB_OK | MB_ICONINFORMATION);
                break;
            }
        }
        ...
        if (HIWORD(wParam) == 0)
        {
            ...
            case IDM_DELETE:
            {
                ...
                op.fFlags = FOF_ALLOWUNDO | FOF_SILENT;
...
void CreateControls(HWND hwnd)
{
    ...
    g_hBtnAbout = CreateWindowExA(0, "BUTTON", "关于软件", WS_VISIBLE | WS_CHILD, DPIScale(870), DPIScale(btnY2), DPIScale(100), DPIScale(35), hwnd, (HMENU)(INT_PTR)ID_BTN_ABOUT, g_hInst, NULL);
